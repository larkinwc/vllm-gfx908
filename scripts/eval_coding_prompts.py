...
def grade(prompt: dict, response: str) -> dict:
    ...
    with tempfile.TemporaryDirectory() as td:
        if lang == "python":
            ...
            r = subprocess.run(
                ["/usr/bin/env", "python3", "-m", "py_compile", str(fpath)],
                capture_output=True,
                text=True,
            )  # noqa: E501
            if r.returncode != 0:
            ...
            if not out["correct"]:
                out["error"] = (
                    f"sanity-call failures: {[r for r in results if not r.get('ok')][:3]}"  # noqa: E501
                )
        ...
        if lang == "javascript":
            ...
            fpath.write_text(code)
            r = subprocess.run(
                ["/usr/bin/env", "node", "--check", str(fpath)],
                capture_output=True,
                text=True,
            )  # noqa: E501
            if r.returncode != 0:
        ...
        if lang == "bash":
            ...
            fpath.write_text(code)
            r = subprocess.run(
                ["bash", "-n", str(fpath)], capture_output=True, text=True
            )  # noqa: E501
            if r.returncode != 0:
            ...
            out["compiles"] = True
            r = subprocess.run(
                ["bash", str(fpath)], capture_output=True, text=True, timeout=10
            )  # noqa: E501
            if r.returncode != 0:
                out["error"] = f"runtime: {r.stderr.strip()[:240]}"
                return out

...
def main() -> int:
    ...
    tuning = json.loads((FINAL_DIR / "tuning_hashes.json").read_text()) if (FINAL_DIR / "tuning_hashes.json").exists() else {}
    ...
    ppl_fp16 = json.loads((Path("/root/bench-int8-w4a16/baseline") / "ppl_fp16.json").read_text())["perplexity"]
    ppl_w8a8_m0 = json.loads((Path("/root/bench-int8-w4a16/baseline") / "ppl_w8a8.json").read_text())["perplexity"]
    ppl_w4a16_m0 = json.loads((Path("/root/bench-int8-w4a16/baseline") / "ppl_w4a16.json").read_text())["perplexity"]
    ppl_w8a8_m4 = json.loads((Path("/root/bench-int8-w4a16/m4") / "ppl_w8a8_m4_ck.json").read_text())["perplexity"]
    ppl_w4a16_m3 = json.loads((Path("/root/bench-int8-w4a16/m3") / "ppl_w4a16_m3.json").read_text())["perplexity"]
    ...
    spotcheck = list(csv.DictReader(spotcheck_path.open())) if spotcheck_path.exists() else []

    lines: list[str] = []
    lines.append("# BENCH_INT8_W4A16_FINAL — gfx908 (MI100) custom kernels, final aggregate")
    lines.append("")
    lines.append("Final Pareto report for the MI100 (gfx908) custom INT8 / W4A16 kernel mission.")
    lines.append("Aggregates milestones M0–M5 into a single grid with per-(cell × metric) winners,")
    lines.append("production recommendations, full quality-gate evidence, and per-cell reproducible")
    lines.append("launch scripts.")
    lines.append("")
    lines.append("Cross-links: ")
    lines.append("[BENCH_INT8_W4A16_BASELINE.md](BENCH_INT8_W4A16_BASELINE.md) (M0+M1), ")
    lines.append("[BENCH_INT8_W4A16_M2.md](BENCH_INT8_W4A16_M2.md) (TensileLite three-way), ")
    lines.append("[BENCH_INT8_W4A16_M3.md](BENCH_INT8_W4A16_M3.md) (Triton W8A8 + W4A16), ")
    lines.append("[BENCH_M4_CK.md](BENCH_M4_CK.md) (Composable Kernel W8A8 + W4A16-negative), ")
    ...
    lines.append("| vLLM | 0.20.2rc1.dev107+gd960f21e4 (mission worktree, editable install) |")
    lines.append("| Models | `/models/Qwen3.5-9B-{w8a8,w4a16}`, FP16 ref `/models/Qwen3.5-9B` |")
    lines.append(f"| Tuning manifest | `/root/bench-int8-w4a16/final/tuning_hashes.json` ({len(tuning)} pinned files) |")
    lines.append("")
    ...
    lines.append(f"| M0 W8A8 (`/models/Qwen3.5-9B-w8a8`) | {ppl_w8a8_m0:.4f} | {d_fp16_w8a8:+.3f} % | — (reference) |")
    d_fp16_w4a16 = (ppl_w4a16_m0 - ppl_fp16) / ppl_fp16 * 100
    lines.append(f"| M0 W4A16 (`/models/Qwen3.5-9B-w4a16`) | {ppl_w4a16_m0:.4f} | {d_fp16_w4a16:+.3f} % | — (reference) |")
    d_m6_w8a8_fp16 = (ppl_w8a8_m6 - ppl_fp16) / ppl_fp16 * 100
    d_m6_w8a8_m0 = (ppl_w8a8_m6 - ppl_w8a8_m0) / ppl_w8a8_m0 * 100
    lines.append(f"| **M6 W8A8 stack (best path per cell)** | **{ppl_w8a8_m6:.4f}** | **{d_m6_w8a8_fp16:+.3f} %** | **{d_m6_w8a8_m0:+.3f} %** |")
    d_m6_w4a16_fp16 = (ppl_w4a16_m6 - ppl_fp16) / ppl_fp16 * 100
    d_m6_w4a16_m0 = (ppl_w4a16_m6 - ppl_w4a16_m0) / ppl_w4a16_m0 * 100
    lines.append(f"| **M6 W4A16 stack (best path per cell)** | **{ppl_w4a16_m6:.4f}** | **{d_m6_w4a16_fp16:+.3f} %** | **{d_m6_w4a16_m0:+.3f} %** |")
    lines.append("")
    lines.append(
        f"- VAL-FINAL-002 gate (Δ ≤ +3 % vs FP16 + Δ ≤ +0.5 % vs M0): "
        f"W8A8 **{'PASS' if d_m6_w8a8_fp16 <= 3.0 and abs(d_m6_w8a8_m0) <= 0.5 else 'FAIL'}** ; "
        f"W4A16 **{'PASS' if d_m6_w4a16_fp16 <= 3.0 and abs(d_m6_w4a16_m0) <= 0.5 else 'FAIL'}**."
    )
    lines.append(
    ...
    lines.append("- Evidence: `/root/bench-int8-w4a16/{baseline,m3,m4,final}/ppl_*.json`.")
    lines.append("")

    # Coding-agent
    lines.append("Coding-agent qualitative pass (10 prompts, rubric compiles/runs/correct):")
    lines.append("")
    ...
    lines.append("")
    lines.append("- Evidence: `tests/eval/coding_prompts.json`, `/root/bench-int8-w4a16/final/m6_coding_eval_m6.{json,md}`.")
    lines.append("- Failures (if any) include diff vs FP16-baseline response in the JSON `response` field.")
    lines.append("")
    ...
    lines.append("")
    if needle is None:
        lines.append("- (not yet run — see `scripts/eval_needle.py --ctx 32768 --probes 5`)")
    else:
        passes = needle["passes"]
        total = needle["total"]
        gate = needle.get("gate_5_of_5")
        lines.append(f"- Result: **{passes}/{total}**, gate {'✅ PASS' if gate else '❌ FAIL'} (5/5 required by VAL-FINAL-004)")
    ...
    lines.append("")
    lines.append("- **stock** = first vLLM run on the artifact (May 7 baseline, commit `fc20b6f4f`).")
    ...
    lines.append("- **+Triton** = M3 custom Triton kernels (`mi100_int8`, `mi100_w4a16`, commit `9699d1f0a`).")
    lines.append("- **+CK** = M4 Composable Kernel `DeviceGemm_Xdl_CShuffle` W8A8 instances "
                 "(commits `a852f2600` → `f4bf9e503`). W4A16 CK declined as negative result "
                 "(see [BENCH_M4_CK.md](BENCH_M4_CK.md)).")
    lines.append("- **+ISA** = n/a (M5 hand-ISA declined; see [BENCH_M5_ISA.md](BENCH_M5_ISA.md)).")
    lines.append("- **winner** = best-of-row across the present paths "
                 "(higher-better for throughput, lower-better for latency).")
    lines.append("")
    lines.append("Workload key: `synth`=synthetic random (1024 in / 256 out, num_prompts=200); "
                 "`coding`=coding-agent realistic. Metric key: `tput`=output toks/s; "
                 "`req_tput`=req/s; `*_ttft`/`*_tpot` in ms.")
    lines.append("")
    lines.append("| cell | workload | metric | stock | +TensileLite | +Triton | +CK | +ISA | winner |")
    lines.append("| --- | --- | --- | ---: | ---: | ---: | ---: | ---: | :-: |")
    for row in rows:
        model = row["cell"].split("_", 1)[0]
        w8a8 = model == "w8a8"
        applies = {"stock": True, "tensilelite": w8a8, "triton": True, "ck": w8a8, "isa": False}
        winner_label_str = winner_label(row["winner_path"], applies)
        higher = row["higher_better"] == "True"
        wl = row["workload"]
        cell_id = row["cell"]
        lines.append(
            f"| {cell_id} | {'synth' if wl=='synthetic' else 'coding'} | {row['metric']} | "
    ...
    lines.append("")
    lines.append("Raw CSV (importable): `/root/bench-int8-w4a16/final/final_grid.csv` (144 rows).")
    lines.append("")
    ...
    lines.append(
        ...
        "Each row carries one-sentence justification + the env flags required to ship the path."
    )
    lines.append("")
    lines.append("| model shape | TP | concurrency | recommended path | justification | env flags |")
    lines.append("| --- | :-: | :-: | --- | --- | --- |")
    ...
    flags_for_path = {
        "stock": "(no flags)",
        "tensilelite": "`HIPBLASLT_TENSILE_LIBPATH=<merged-library>` ; default dispatcher",
        "triton": "`VLLM_ROCM_USE_AITER=1` (default in services.yaml)",
        "ck": "`VLLM_ROCM_USE_AITER=1` ; default dispatcher prefers CK > hipBLASLt > Triton",
    ...
    for cell, bucket in sorted(by_cell.items()):
        ...
        if runner_up:
            ...
            just = (
                f"{pretty[wp]} wins on tput-geomean over {pretty[ru_p]} by {delta:+.2f} % "
        ...
        if wp == "stock" and any(p in means for p in ("ck", "triton")):
            just += (
                ...
                "kept stock May-7 column slightly ahead; see [BENCH_INT8_W4A16_M2.md](BENCH_INT8_W4A16_M2.md).)"
            )
        lines.append(
    ...
    lines.append("")
    lines.append(
        "Per-cell launch scripts at `scripts/launch_<model>_tp<tp>_c<conc>.sh` (12 scripts). "
        "Each pins env vars (ROCm 7.12, PYTORCH_ROCM_ARCH=gfx908, vLLM commit, tuning JSON path), "
        "starts the matching vLLM service, runs the canonical 200-prompt synthetic bench, and "
    ...
    if spotcheck:
        ...
        for row in spotcheck:
            in2 = (row.get("within_band_2pct") or row.get("within_band") or "False").lower() == "true"
        ...
        lines.append(
            "Spot-check evidence: `/root/bench-int8-w4a16/final/m6_repro_spotcheck.csv` "
    ...
    lines.append(
        ...
        "Triton W8A8/W4A16 correctness `tests/kernels/quantization/test_mi100_int8_correctness.py`, "
    ...
    lines.append(
        ...
        "Every JSON under `/root/bench-int8-w4a16/**/{synthetic,coding}/*.json` includes "
        "`launch_command`, `env`, `vllm_commit`, `rocm_version`, `torch_version`, `triton_version`, "
    ...
    lines.append(
        ...
        "`scripts/check_forbidden_intrinsics.sh build/**/*.so` reports zero matches across "
        "`_C.abi3.so`, `_moe_C.abi3.so`, `_rocm_C.abi3.so`. |"
    )
    lines.append(
        "| VAL-CROSS-005 dispatcher priority CK > hipBLASLt > Triton | ✅ | "
        "M4 dispatch verification at `/root/bench-int8-w4a16/m4/wiring_verification.txt` "
        "(symlinked from `/root/bench-int8-w4a16/ck/m4_dispatch_trace.txt`); contention test "
        "`tests/kernels/quantization/test_ck_dispatch_priority.py`. |"
    )
    lines.append(
        "| VAL-CROSS-006 no git pushes | ✅ | "
        "`git reflog --date=iso \\| grep push` returns empty; all milestone commits are local. |"
    )
    lines.append(
        "| VAL-CROSS-007 tuning-JSON SHA256 provenance | ✅ | "
        "`scripts/verify_tuning_hashes.py` compares on-disk SHA256 of every pinned tuning JSON "
    ...
    lines.append("    - `/root/bench-int8-w4a16/baseline/profile/omniperf/omniperf_summary_{w8a8,w4a16}.json`")
    ...
    lines.append("")
    lines.append("Run `scripts/validate_final_report.py --check-links BENCH_INT8_W4A16_FINAL.md` "

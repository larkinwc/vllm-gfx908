...
| **torch.compile + FULL_AND_PIECEWISE** (concurrency-gated) | `scripts/launch_compile_*.sh` (`VLLM_MI100_TORCH_COMPILE=1` + `VLLM_MI100_ALLOW_PIECEWISE=1` + `mode=3`); TP=4 only | MoE +9–11 % all-c (fp16 35B & W4A16 512e, no c=1 loss); dense 9B +4.5/+7.2 % at TP=4 (regresses TP=1 — not shipped). Needs the Dynamo guard fix (`24ac8f64e`) | [`BENCH_FP16_COMPILE_MOE_2026_06.md`](docs/experiments/BENCH_FP16_COMPILE_MOE_2026_06.md), [`BENCH_FP16_COMPILE_QUANT_MOE_2026_06.md`](docs/experiments/BENCH_FP16_COMPILE_QUANT_MOE_2026_06.md), [`BENCH_FP16_COMPILE_PORT_0202_2026_06.md`](docs/experiments/BENCH_FP16_COMPILE_PORT_0202_2026_06.md) |
| **W8A16 (GPTQ-8bit) capacity + compile** | `scripts/launch_w8a16_moe_tp4_c{1,2,4}.sh` (`/models/Qwen3.5-35B-A3B-GPTQ-8bit`) | 8-bit weight-only keeps FP16 MFMA path; 35B-A3B MoE fits + serves on **2** MI100s (18.4 GiB/GPU, 626k-tok KV) vs fp16 needing 4. PPL 7.45, coding 9/10, needle 5/5. Same-binary compile A/B (v0.22.1, TP=2): **+14.2 % c1 / +40.4 % c2 / +48.1 % c4** | [`BENCH_W8A16_35B_MOE_2026_06.md`](docs/experiments/BENCH_W8A16_35B_MOE_2026_06.md) |
| **Tuned int8_w8a16 E=256/N=256 MoE config** (concurrency-gated, opt-in `LAUNCH_TUNED_MOE=1`) | `scripts/moe_configs_w8a16/` via `VLLM_TUNED_CONFIG_FOLDER` | From-scratch tune of the W8A16 N=256 shape. Server A/B (v0.22.1, TP=2): **−11.6 % c1 / +15.3 % c2 / +23.7 % c4** — net win at c≥2, regresses single-stream, so **not** installed globally. Prefill-safe (0 losses M>32). Unlike int4/N=128 (no headroom) this N=256 shape has real headroom | [`BENCH_MOE_TUNE_W8A16_E256N256_2026_06.md`](docs/experiments/BENCH_MOE_TUNE_W8A16_E256N256_2026_06.md) |
...
| **DFlash speculative decoding** | Rejected | Functionally correct on gfx908 (non-causal attn + CUDA graphs validated, ~2.0 steady-state acceptance length), but **net-negative**: 0.37–0.53× baseline tput across synthetic + coding c=1/2/4, confirmed on two GPU pairs. MI100's compute-bound FP16 decode can't absorb the wider batched verify. | [`BENCH_DFLASH_GFX908.md`](docs/experiments/BENCH_DFLASH_GFX908.md) |

...
| 2026-06 | **Ship torch.compile as gated launch scripts** (`scripts/launch_compile_*.sh`, TP=4 only) | WIN ⇒ shipped. MoE +9–11 % all-c (fp16 35B & W4A16 512e); dense 9B +4.5/+7.2 % TP=4 (TP=1 regresses, not shipped). Dynamo guard `24ac8f64e` confirmed on a 4th model (W8A16 35B) | `BENCH_W8A16_35B_MOE_2026_06.md` + compile series |
| 2026-06 | **W8A16 (GPTQ-8bit) 35B-A3B capacity + compile A/B** | WIN ⇒ shipped `launch_w8a16_moe_tp4_*`. 35B MoE fits on **2** MI100s (18.4 GiB/GPU, 626k-tok KV) vs fp16 needing 4; PPL 7.45, coding 9/10, needle 5/5. Same-binary compile A/B (rebuilt v0.22.1, TP=2): **+14.2 % c1 / +40.4 % c2 / +48.1 % c4** — c1 positive here (vs 0.20.2 dense c1 loss). Missing `int8_w8a16 E=256,N=256` tuned MoE config understates absolute tput (both arms equal) | `BENCH_W8A16_35B_MOE_2026_06.md` |
| 2026-06 | **Tune int8_w8a16 E=256/N=256 MoE from scratch** (the W8A16 N=256 shape; contrast int4/N=128 = no headroom) | **Concurrency-gated WIN ⇒ shipped opt-in** (`LAUNCH_TUNED_MOE=1` → `VLLM_TUNED_CONFIG_FOLDER=scripts/moe_configs_w8a16/`, **not** global). Server A/B (v0.22.1, TP=2): **−11.6 % c1 / +15.3 % c2 / +23.7 % c4**; net win at c≥2, regresses single-stream. Kernel A/B geomean 0.914 (tie M≤16, win M≥24); prefill-safe (0 losses M>32, unlike inherited int4 −38 %). Win is partly a bad default (`BLOCK_M=32` at M≥24 hurts N=256). Also fixes 3 stale-API bugs in `benchmark_moe.py` int8_w8a16 path | `BENCH_MOE_TUNE_W8A16_E256N256_2026_06.md` |
